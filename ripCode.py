...
def calibrateMPU6050 (dest1, dest2):
    ...
    writeByte(_EN, 0x00)  # Disable gyro and accelerometer sensors for FIFO
    ...
    accel_temp[1] = (int16_t)(((int16_t)
    data[2] << 8) | data[3]  )
    accel_temp[2] = (int16_t)(((int16_t)
    data[4] << 8) | data[5]  )
    gyro_temp[0] = (int16_t)(((int16_t)
    data[6] << 8) | data[7]  )
    gyro_temp[1] = (int16_t)(((int16_t)
    data[8] << 8) | data[9]  )
    gyro_temp[2] = (int16_t)(((int16_t)
    data[10] << 8) | data[11])

    accel_bias[0] += (int32_t)
    accel_temp[0]  # Sum individual signed 16-bit biases to get accumulated signed 32-bit biases
    accel_bias[1] += (int32_t)
    accel_temp[1]
    ...
    int32_t
    accel_bias_reg[3] = [0, 0, 0]  # A place to hold the factory accelerometer trim biases
    readBytes(MPU6050_ADDRESS, XA_OFFSET_H, 2, & data[0])  # Read factory accelerometer trim values
    accel_bias_reg[0] = (int16_t)((int16_t)
    data[0] << 8) | data[1]
    readBytes(MPU6050_ADDRESS, YA_OFFSET_H, 2, & data[0])
    accel_bias_reg[1] = (int16_t)((int16_t)
    data[0] << 8) | data[1]
    readBytes(MPU6050_ADDRESS, ZA_OFFSET_H, 2, & data[0])
    accel_bias_reg[2] = (int16_t)((int16_t)
    data[0] << 8) | data[1]

    uint32_t
    mask = 1
    uL  # Define mask for temperature compensation bit 0 of lower byte of accelerometer bias registers
    uint8_t
    mask_bit[3] = [0, 0, 0]  # Define array to hold mask bit for each accelerometer bias axis
    ...
    factoryTrim[0] = (4096.0 * 0.34) * (pow((0.92 / 0.34), (((float)
    selfTest[0] - 1.0) / 30.0)))  # FT[Xa] factory trim calculation
    factoryTrim[1] = (4096.0 * 0.34) * (pow((0.92 / 0.34), (((float)
    selfTest[1] - 1.0) / 30.0)))  # FT[Ya] factory trim calculation
    factoryTrim[2] = (4096.0 * 0.34) * (pow((0.92 / 0.34), (((float)
    selfTest[2] - 1.0) / 30.0)))  # FT[Za] factory trim calculation
    factoryTrim[3] = (25.0 * 131.0) * (pow(1.046, ((float)
    selfTest[3] - 1.0) ))  # FT[Xg] factory trim calculation
    factoryTrim[4] = (-25.0 * 131.0) * (pow(1.046, ((float)
    selfTest[4] - 1.0) ))  # FT[Yg] factory trim calculation
    factoryTrim[5] = (25.0 * 131.0) * (pow(1.046, ((float)
    selfTest[5] - 1.0) ))  # FT[Zg] factory trim calculation
